...
const CatalogItemRelationshipController = require('./controllers/catalog-item-relationship-controller');
...
module.exports = function (bus, options) {
	...
	const service = {
		...
		CatalogItemRelationshipController,
	...
	service.router = function (options) {
		...
		types.forEach(type => {
			...
			router.all(
				`/${type}s/:id/relationships/:relationshipKey`,
				middleware['request-authorize']({bus, audience: {
					get: ['admin', 'platform']
				}}),
				CatalogItemRelationshipController.create({bus, type})
			);
		});
