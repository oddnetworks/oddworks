...
const CatalogSpecItemController = require('./controllers/catalog-item-controller');
const CatalogSpecListController = require('./controllers/catalog-list-controller');
...
exports.initialize = (bus, options) => {
	...
	if (!identityService || !_.isObject(identityService)) {
		throw new Error('options.identityService is required.');
	}
	...
	service.router = function (options) {
		options = _.defaults({}, options, {
			...
			specTypes: []
			// example: 'collectionSpec', 'promotionSpec', 'videoSpec', 'viewSpec'
		});
		...
		types.forEach(type => {
			router.all(
				`/${type}s`,
				identityService.middleware.authorize({audience: {
					get: ['admin', 'platform'],
					patch: ['admin'],
					delete: ['admin']
				}}),
				CatalogListController.create({
					bus: service.bus,
					type
				})
			);
		...
		specTypes.forEach(type => {
			router.all(
				...
				CatalogSpecListController.create({
					bus: service.bus,
					type
				})
			);
			...
			router.all(
				...
				CatalogSpecItemController.create({
					bus: service.bus,
					type
				})
			);
