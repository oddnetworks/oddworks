...
module.exports = function (service) {
	...
	const UPDATE_FREQ = service.options.updateFrequency || 1;
	...
	function checkCache(object) {
		...
		if (object.meta.hasSpec && maxAge) {
			...
			const updateFrequency = object.meta.updateFrequency || UPDATE_FREQ;
	...
	queries.fetchItemSource = function fetchItemSource(object) {
		if (!object.channel || !_.isString(object.channel)) {
			throw new Error('fetchItemSource() expects object.channel to be present');
		}
		if (!object.type || !_.isString(object.type)) {
			throw new Error('fetchItemSource() expects object.type to be present');
		}
		if (!object.id || !_.isString(object.id)) {
			throw new Error('fetchItemSource() expects object.id to be present');
		}
		...
		return queries.fetchItemSpec({channel, type, id})
			.then(spec => {
				if (!spec) {
					return null;
				}
	...
	queries.fetchItemSpec = function fetchItemSpec(args) {
		...
		if (!args.channel || !_.isString(args.channel)) {
			throw new Error('fetchItemConfig() args.channel is required');
		}
		if (!args.type || !_.isString(args.type)) {
			throw new Error('fetchItemConfig() args.type is required');
		}
		if (!args.id || !_.isString(args.id)) {
			throw new Error('fetchItemConfig() args.id is required');
		}

		const channel = args.channel;
		const type = `${args.type}_config`;
		const id = args.id;
