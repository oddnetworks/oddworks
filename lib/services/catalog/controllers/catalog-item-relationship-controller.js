...
class CatalogItemRelationshipController extends Controller {
	...
	get(req, res, next) {
		...
		return this.getChannel(req)
			...
			.then(resource => {
				if (!resource || !_.get(resource, 'relationships') || !_.get(resource, `relationships.${relationshipKey}`)) {
					return Promise.reject(Boom.notFound(`Resource or relationship "${relationshipKey}" not found for ${type}.`));
				}
