	...
	get(req, res, next) {
		...
		return this.getChannel(req)
			...
			.then(resource => {
				...
				object = _.cloneDeep(resource);

				let progressPromise = Promise.resolve(false);
				if (viewer && object.type === 'video') {
					progressPromise = this.bus.query({role: 'store', cmd: 'get', type: 'progress'}, {id: `${object.id}:${viewer.id}`, type: 'progress', channel: channel.id});
				}
			...
			.then(progress => {
				if (progress) {
					object.position = progress.position || 0;
					object.complete = progress.complete || false;
				}

				let includePromise = Promise.resolve(false);
				if (object.included) {
					included = _.cloneDeep(object.included);

					includePromise = Promise.map(included, resource => {
						if (viewer && resource.type === 'video') {
							const viewerId = (viewer || {}).id;
							return this.bus.query({role: 'store', cmd: 'get', type: 'progress'}, {id: `${resource.id}:${viewerId}`, type: 'progress', channel: channel.id})
								.then(progress => {
									progress = progress || {};
