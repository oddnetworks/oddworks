		...
		return this.getChannel(req)
			...
			.then(resource => {
				...
				if (viewer && resource.type === 'video') {
					progressPromise = this.bus.query({role: 'store', cmd: 'get', type: 'progress'}, {id: `${viewer.id}:${resource.id}`, type: 'progress', channel: channel.id});
				}
			...
			.then(progress => {
				...
				if (res.body.included) {
					includePromise = Promise.map(res.body.included, resource => {
						return this.bus.query({role: 'store', cmd: 'get', type: 'progress'}, {id: `${viewer.id}:${resource.id}`, type: 'progress', channel: channel.id})
							.then(progress => {
								if (progress) {
