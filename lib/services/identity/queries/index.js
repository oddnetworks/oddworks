...
module.exports = function (service) {
	...
	queries.composeConfig = function composeConfig(args) {
		...
		const channelFeatures = _.keys(channel.features);
		const platformFeatures = _.keys(platform.features);
		const features = _.union(channelFeatures, platformFeatures);

		_.each(features, key => {
			const channelKey = _.get(channel, `features.${key}`);
			const platformKey = _.get(platform, `features.${key}`);
			const mergedKey = _.merge({}, channelKey, platformKey);

			_.set(config, `features.${key}`, mergedKey);
		});
