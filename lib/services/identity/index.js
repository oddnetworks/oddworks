...
exports.router = function (options) {
	...
	types.forEach(type => {
		router.all(
			`/${type}s`,
			exports.middleware.authorize({audience: {
				get: ['admin'],
				post: ['admin']
			}}),
			IdentityListController.create({bus: SERVICE.bus, type})
		);
		...
		router.all(
			...
			IdentityItemController.create({bus: SERVICE.bus, type})
		);
	});
	...
	router.all(
		...
		IdentityConfigController.create({bus: SERVICE.bus})
	);
