...
function issueUserCode(req, res, next, done) {
	...
	var networkID = req.identity.network.id;
	var deviceType = req.identity.device.deviceType;

	seneca.act({role: 'device-auth', cmd: 'issueUserCode', networkID: networkID, deviceType: deviceType}, function (err, codeResponse) {
		if (err) {
			switch (err.name) {
				case 'CacheError':
...
function authorizeUser(req, res, next, done) {
	...
	seneca.act({role: 'device-auth', cmd: 'authorizeUser', networkID: networkID, entitlementCredentials: entitlementCredentials, userCode: userCode}, function (err, authorizeResponse) {
		if (err) {
			switch (err.name) {
				case 'CacheError':
					return next(boom.badImplementation(err.message));
...
function issueAccessToken(req, res, next, done) {
	...
	seneca.act({role: 'device-auth', cmd: 'issueAccessToken', networkID: networkID, deviceType: deviceType, deviceCode: deviceCode}, function (err, tokenResponse) {
		if (err) {
			switch (err.name) {
				case 'CacheError':
					return next(boom.badImplementation(err.message));
...
function listUserDevices(req, res, next, done) {
	...
	seneca.act({role: 'device-auth', cmd: 'listUserDevices', networkID: networkID, clientUserID: clientUserID}, function (err, userDevices) {
		if (err) {
			switch (err.name) {
				case 'DatabaseError':
					return next(boom.badImplementation(err.message));
...
function removeUserDevice(req, res, next, done) {
	...
	seneca.act({role: 'device-auth', cmd: 'removeUserDevice', networkID: networkID, clientUserID: clientUserID, deviceUserProfileID: deviceUserProfileID}, function (err, tokenResponse) {
		if (err) {
			switch (err.name) {
				case 'DatabaseError':
					return next(boom.badImplementation(err.message));
