...
module.exports = {
	SwapPCLiveStreamForIntl: function (req, res) {
		if (req.identity.organization.id === 'poker-central' && req.params.entityType && req.params.entityType === 'views' && res.get('X-Geo-Country-Code') !== 'US') {
	...
	LimitPCXboxOneSearchResults: function(req, res) {
		if (req.identity.organization.id === 'poker-central' && req.identity.device.deviceType === 'MICROSOFT_XBOXONE') {
	...
	AttachOldRokuAdUrl: function(req, res) {
		if (req.identity.organization.id === 'poker-central' && req.identity.device.deviceType === 'ROKU_ROKU') {
			req.data.ads = {
