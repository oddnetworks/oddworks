...
class Controller {
	// TODO: Finish updating this Class - @kixxauth
	// isAdminRequest(req) {
	...
	fetchChannel(args) {
		...
		if (args.req.identity.channel) {
			...
			return Promise.reject(Boom.badData('"channel" is required'));
		}
		return promise;
	}
	...
	getFetchChannel(args) {
		...
		if (!channelId && (args.req.identity.audience || []).indexOf('admin') >= 0) {
			...
			if (!channelId) {
				return Promise.reject(Boom.badData('On admin get request, query.channel is required'));
			}
		}
	...
	patchFetchChannel(args) {
		...
		if (!channelId && (args.req.identity.audience || []).indexOf('admin') >= 0) {
			...
			if (!channelId) {
				return Promise.reject(Boom.badData('On admin patch request, a channel object on the payload is required'));
			}
		}
		args.channelId = channelId;
		return this.fetchChannel(args);
	...
	patchIdentityFetchChannel(args) {
		...
		if (!channelId && (args.req.identity.audience || []).indexOf('admin') >= 0) {
			...
			if (!channelId) {
				return Promise.reject(Boom.badData('On admin patch request for an identity object, a payload.channel or payload.id is required'));
			}
		}
		args.channelId = channelId;
		return this.fetchChannel(args);
	...
	postFetchChannel(args) {
		...
		if (!channelId && (args.req.identity.audience || []).indexOf('admin') >= 0) {
			...
			if (!channelId) {
				return Promise.reject(Boom.badData('On admin post request, payload.channel is required'));
			}
		}
		args.channelId = channelId;
		return this.fetchChannel(args);
