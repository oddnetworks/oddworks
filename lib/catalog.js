...
function getEntity(req, res, next, done) {
	...
	act({role: 'catalog', cmd: cmd, id: entityId, organization: req.identity.organization, device: req.identity.device, user: req.identity.user})
			...
			if (entity.type === 'video') {
				...
				return Promise.resolve(entity)
					.then(function fetchRelatedEntities(entity) {
						return act({role: 'catalog', cmd: 'related', depth: depth, entity: entity, organization: req.identity.organization, device: req.identity.device, user: req.identity.user});
					})
...
function getEntityRelationship(req, res, next, done) {
	...
	act({role: 'catalog', cmd: cmd, id: entityId, depth: depth, organization: req.identity.organization, device: req.identity.device, user: req.identity.user})
		.then(function fetchRelatedEntities(entity) {
			if (!entity) {
			...
			return Promise.all(_.map(relatedEntities, function fetchMultipleRelatedEntities(relatedEntity) {
				...
				return act({role: 'catalog', cmd: relatedEntityCmd, id: relatedEntity.id, depth: depth,
					organization: req.identity.organization, device: req.identity.device});
			}));
		})
		...
		.then(function fetchDeepRelatedEntities(relatedResults) {
			if (isSingleEntity) {
				return act({role: 'catalog', cmd: 'related', depth: depth, entity: relatedResults,
					organization: req.identity.organization, device: req.identity.device, user: req.identity.user});
			}
