...
module.exports = function (options) {
	...
	const repsonseJsonApi = (req, res, next) => {
		...
		if (isSearchRoute(req)) {
			...
			_.set(res.body, 'meta.query.q', q);

			const types = _.get(req, 'query.types', 'video,collection').split(',');
			_.set(res.body, 'meta.query.types', types);

			const limit = _.get(req, 'query.page.limit', 10);
			_.set(res.body, 'meta.page.limit', limit);

			const offset = _.get(req, 'query.page.offset', 0);
			_.set(res.body, 'meta.page.offset', offset);

			const sortBy = _.get(req, 'query.sort', null);
