...
exports.initialize = function createMemoryStore(bus, options) {
	...
	const cache = types.reduce((store, type) => {
		store[type] = Object.create(null);
		return store;
	}, Object.create(null));

	function createSetter(type) {
		const table = cache[type] || Object.create(null);
	...
	function createGetter(type) {
		...
		return function get(args) {
			...
			if (entity && include && include.length) {
				...
				.flatten()
				.map(item => {
					const table = cache[item.type] || Object.create(null);
	...
	function batchGet(args) {
		if (!Array.isArray(args)) {
			throw new Error('batchGet() args must be an Array.');
		}
		...
		const results = args.map(arg => {
			const table = cache[arg.type] || Object.create(null);
			const id = arg.id;
			// Make a copy to prevent unintended mutation.
