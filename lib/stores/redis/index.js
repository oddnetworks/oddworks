...
exports.initialize = function createRedisStore(bus, options) {
	...
	function createKey(object) {
		if (object.channel) {
			return `${object.channel}:${object.type}:${object.id.slice(0, 2)}`;
		}
		return `${object.type}:${object.id.slice(0, 2)}`;
	}
	...
	const hset = (function () {
		...
		if (_.isFunction(redis.hsetAsync)) {
			hsetAsync = object => {
				...
				return redis
					.hsetAsync(key, object.id, JSON.stringify(object))
			...
			hsetAsync = object => {
				return new Promise((resolve, reject) => {
					const key = createKey(object);
					redis.hset(key, object.id, JSON.stringify(object), err => {
						if (err) {
							return reject(err);
						}
						resolve(object);
	...
	const hget = (function () {
		...
		if (_.isFunction(redis.hgetAsync)) {
			hgetAsync = object => {
				const key = createKey(object);
				return redis.hgetAsync(key, object.id).then(res => {
			...
			hgetAsync = object => {
				return new Promise((resolve, reject) => {
					const key = createKey(object);
					redis.hget(key, object.id, (err, res) => {
						if (err) {
							return reject(err);
						}
	...
	const hscan = (function () {
		...
		return function redisHscan(args) {
	...
	bus.queryHandler(
		{role: 'store', cmd: 'batchGet', type: 'channel'},
		batchGetChannel
	);
