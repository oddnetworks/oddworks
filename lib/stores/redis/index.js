...
module.exports = function (bus, options) {
	...
	const hscan = (function () {
		...
		if (_.isFunction(redis.dbsizeAsync)) {
			dbsizeAsync = () => {
				return redis.dbsizeAsync();
			};
		} else {
			dbsizeAsync = () => {
				return new Promise((resolve, reject) => {
					return redis.dbsize((err, res) => {
						if (err) {
							return reject(err);
						}
		...
		const redisScan = args => {
			...
			const lastEvaluatedKey = (args.lastEvaluatedKey || {}).key;
			const lastCursor = (args.lastEvaluatedKey || {}).cursor || 0;

			const sort = args.sort;
			const descending = Boolean(args.descending);

			return dbsizeAsync().then(dbsize => {
				// Determine the scan page size by the size of the database
				// (the total number of keys in the database).
				let SCAN_PAGE_SIZE = Math.ceil(dbsize / SCAN_PAGES);
				SCAN_PAGE_SIZE = SCAN_PAGE_SIZE > limit ? SCAN_PAGE_SIZE : limit;
						...
						.then(items => {
							...
							if (lastEvaluatedKey) {
								start = _.findIndex(items, item => {
									return item.id === lastEvaluatedKey;
								});

								// Add 1 to the last evaluated key index (so it's exclusive).
								start += 1;
							...
							return {results, lastEvaluatedKey: {key, cursor}};
						});
				}
				...
				return fetchKeysLimited(pattern, lastCursor, SCAN_PAGE_SIZE, limit)
					...
					.then(results => {
						...
						return {results, lastEvaluatedKey: {cursor}};
					});
			});
		};
	...
	function createScanner(type) {
		const scan = args => {
			...
			return hscan({channel: args.channel, type, limit, lastEvaluatedKey: args.lastEvaluatedKey});
		};
