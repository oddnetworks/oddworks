...
module.exports = function (bus, options) {
	...
	const hscan = (function () {
		...
		// TODO: Optional limit to keys retrieved.
		function fetchAllKeys(pattern, pageSize) {
			function fetchPage(keys, cursor, pageSize) {
		...
		const redisScan = args => {
			...
			let lastEvalutatedKey = (args.lastEvalutatedKey || {}).key;
			let lastCursor = (args.lastEvalutatedKey || {}).cursor || 0;
			...
			// Multiply the limit by 2 to make sure we have enough results to
			// satisfy last evaluated key if there is one.
			const pageSize = args.lastEvalutatedKey ? (limit * 2) : limit;

			// The next cursor
			let cursor;

			// TODO: We need to scan up to the items we need but not too much beyond.
			return scanAsync(pattern, lastCursor, pageSize)
				.then(res => {
					// The next cursor
					cursor = res.cursor;
