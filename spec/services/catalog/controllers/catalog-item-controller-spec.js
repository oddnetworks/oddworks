...
describe('Catalog Item Controller', function () {
	...
	describe('PATCH', function () {
		...
		describe('platform request', function () {
			...
			it('rejects with a forbidden error', function () {
				expect(execError instanceof Error).toBe(true);
				expect(execError.message).toBe(`Non admins may not update resources`);
				const payload = _.get(execError, 'output.payload', {});
				expect(payload.statusCode).toBe(403);
			});
		});
		...
		describe('admin request', function () {
			...
			describe('when not found', function () {
				...
				beforeAll(function (done) {
					spyOn(bus, 'query').and.returnValues(
						Promise.resolve({type: 'channel', id: payload.channel}),
						Promise.resolve(null)
					);
			...
			describe('when found', function () {
				...
				beforeAll(function (done) {
					...
					spyOn(bus, 'sendCommand').and.callFake(function (pattern, args) {
						return args;
					});
				...
				it('calls catalog setItem to update the resource', function () {
					...
					expect(resource).toEqual({channel: 'channel-id', type: 'video', id: 'VIDEO_ID', list: [7, 8]});
				});
				...
				it('assigns the resource to the body', function () {
					expect(res.body.type).toBe(record.type);
	...
	describe('DELETE', function () {
		...
		describe('platform request', function () {
			...
			it('rejects with a forbidden error', function () {
				expect(execError instanceof Error).toBe(true);
				expect(execError.message).toBe(`Non admins may not delete resources`);
				const payload = _.get(execError, 'output.payload', {});
				expect(payload.statusCode).toBe(403);
			});
		});
