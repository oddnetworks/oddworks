...
const CatalogSpecController = require('../../../../lib/services/catalog/controllers/catalog-spec-controller');

describe('Catalog Item Controller', function () {
	const TYPES = Object.freeze(['videoSpec', 'viewSpec', 'collectionSpec', 'trumpetSpec']);
	let bus;
	...
	beforeAll(function () {
		...
		bus.queryHandler({role: 'catalog', cmd: 'fetchItemSpec'}, args => {
		...
		bus.commandHandler({role: 'catalog', cmd: 'removeItemSpec'}, () => {
			return Promise.resolve(true);
		});

		handler = CatalogSpecController.create({bus, type});
	});
		...
		describe('as admin role', function () {
			...
			describe('with channel in query parameter and JWT', function () {
				...
				it('queries the resource through the store insetad of the catalog', function () {
					expect(bus.query).toHaveBeenCalledTimes(2);
					expect(bus.query.calls.argsFor(1)[0]).toEqual({role: 'catalog', cmd: 'fetchItemSpec'});
			...
			describe('with channel in JWT only (not query parameter)', function () {
				...
				it('queries the resource through the store insetad of the catalog', function () {
					...
					expect(bus.query.calls.argsFor(0)[0]).toEqual({role: 'catalog', cmd: 'fetchItemSpec'});
	...
	describe('PATCH', function () {
		...
		describe('as platform role', function () {
			...
			describe('with valid request', function () {
				...
				it('queries for the resource', function () {
					...
					expect(args.channel).toEqual({type: 'channel', id: 'a-channel-id'});
					expect(args.type).toBe(type);
					expect(args.id).toBe('record-id');
				});
		...
		describe('as admin role', function () {
			...
			describe('when the channel does not exist', function () {
				...
				it('only queries for the channel', function () {
					expect(bus.query).toHaveBeenCalledTimes(1);
					expect(bus.query.calls.argsFor(0)[0]).toEqual({role: 'store', cmd: 'get', type: 'channel'});
				});
			...
			describe('with a valid request', function () {
				...
				it('queries for the resource', function () {
					...
					expect(args[0]).toEqual({role: 'catalog', cmd: 'fetchItemSpec'});
					expect(args[1]).toEqual({
						channel: {type: 'channel', id: 'attribute-channel-id'},
						type,
						id: BODY.id
					});
				});
		...
		describe('as platform role', function () {
			...
			describe('when resource exists', function () {
				...
				it('removes the resource', function () {
					expect(bus.sendCommand).toHaveBeenCalledTimes(1);
					expect(bus.sendCommand.calls.argsFor(0)[0]).toEqual({role: 'catalog', cmd: 'removeItemSpec'});
